...
sleep 1
sudo dfu-util -D /tmp/flycron-8.bin -a 0 -R -s 0x08000000

read -p "Press reset on front left, then hit enter..."

stty -F /dev/serial/by-id/usb-Armchair_Heavy_Industries_Flycron_1F004E001351303239373335-if00 1200
sleep 1
sudo dfu-util -D /tmp/flycron-25.bin -a 0 -R -s 0x08000000

read -p "Press reset on rear left, then hit enter..."

stty -F /dev/serial/by-id/usb-Armchair_Heavy_Industries_Flycron_45006C000151333138363230-if00 1200
sleep 1
sudo dfu-util -D /tmp/flycron-8.bin -a 0 -R -s 0x08000000

read -p "Press reset on rear right, then hit enter..."
